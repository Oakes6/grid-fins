...
float apogee = 4500;                        //desired final height
...
int initialAltServoPosition = 145;            //initial angle for servos set to hug the rocket
...
if(!gyro.begin())
  ...
  //servo set function
  int pos = 0;
  for (pos = initialAltServoPosition; pos >= 55; pos--) {
    altServo.write(pos);
    delay(5);
  }
  for (pos = 55; pos <= initialAltServoPosition; pos++) {
    altServo.write(pos);
    delay(50);
  }
  Serial.println("yo 3");
...
//****** Prints telemetric data to card
  ...
  if (hasBurnedOut && velZ < 350) {

    //servo commands go in here
    //increase pitch -- logic reversed for new servos
    ...
    //decrease pitch -- logic reversed for new servos
